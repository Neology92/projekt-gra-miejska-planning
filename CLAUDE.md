...
- **Ostateczne pliki do druku** (wyrenderowane PDF z `prototype/print/render.ps1`) → folder **`public/`** w korzeniu repo. Jedno miejsce na gotowe deliverable do wydruku (przez prototyp→MVP). Źródła renderu (HTML/CSS) zostają w `prototype/print/src/`; `public/` trzyma tylko wynik. Folder jest **gitignorowany** (rendery są regenerowalne; nie wersjonujemy, by uniknąć konfliktów PDF). ⚠ jeśli wolisz wersjonować gotowe wydruki — usuń `public/` z `.gitignore`.
