...
49. **Dźwięk dzwonu** — ROZSTRZYGNIĘTE [2026-06-03, Oskar]: **bez dedykowanego rekwizytu dźwiękowego.** „Ósme bicie" zostaje wyłącznie jako **narracyjny** znacznik czasu finału (koperta Z7 + scenka) — nie produkujemy efektu dźwiękowego. (Anachronizm Tuba Dei pozostaje rozwiązany kanonem: dzwonu nie nazywamy — `lore/fakty-vs-fabula.md` bez zmian.)
...
77. **Z3 (TR) — brama liczbowa u Albrechta + „kod" `30` → APKA** — ROZSTRZYGNIĘTE [2026-06-03, Oskar]: zwiad z K3 dostaje **konkretny cel: wyciągnąć od Albrechta liczbę obrońców zamku**; Albrecht z pychy ją wyjawia (karta **STEP 1B**). ✅ **Liczba = `30`** (dokładnie — „liczba do wpisania") — ⚠ historyczność: toruński zamek ~30 braci (`lore/stronnictwa.md`, `plotki-i-pogloski.md B6`); **świadomie NIE 300** (300 = bracia w *całych Prusach*; Oskar wybrał dosłowność zamku). ✅ **DOWNSTREAM ROZSTRZYGNIĘTY:** kod `30` gracz **wpisuje do APLIKACJI** (`torun-1454-gra.netlify.app`), a apka odsłania rozkaz kradzieży (dawne K-Z3b) — **zamiast fizycznej koperty od Albrechta.** Komtur nic nie wręcza. Zsynchronizowane: `albrecht-actor-card §STEP 1B/2`, `envelopes/miasto-3-Z3.md`, `envelopes/miasto-4-Z3b.md` (delivery=apka), `zamek-krzyzacki-lista.md §Struktura Z3`, `mg-quick-hints §Z3`, `koperty-kody-lista.md` (5× `-4-Z3b` → cięte, 65→60 kopert). ✅ **ZBUDOWANE W APCE [2026-06-03] — OBA TORY:** rozejście frakcji na Z2 (`fork:{TR:'z3',KZ:'z3z'}`). **TR:** etap `z3` bramka **`type:'code'` (answer=`30`)** → `z3b` (rozkaz kradzieży). **KZ:** etap `z3z` bramka **`type:'observe'`** (pola: kolor szaty `brown` + atrybut `ladle`, accept-listy z synonimami) → `z3zb` (handoff do MG). Pliki: `app/steps.js`, `app/app.js` (`codePuzzle`/`observePuzzle`/`forkPanel`/`showSuccess` override), `app/styles.css` (`.field-label`), `app/README.md`. Przetestowane headless (Chromium): full flow TR Z1→Z3b + KZ Z1→Z3Zb, walidacja błędnych/pustych pól, 0 błędów JS. ⚠ KZ: rozkaz Albrechta + Tabele 1/2 zostają FIZYCZNE w kopercie `krzyzacy-3-Z3Z` (apka = skrót + bramka). Brąz+chochla = twarda zależność (kostium Piotra, Tabela 1 Z11). **Deploy:** dopiero po merge do `master` (`.github/workflows/deploy-app-netlify.yml`) — z gałęzi nie publikuje. Patrz też #78.

78. **Kodowanie dalszych zadań — system bramek na dane od NPC** — OTWARTE [zgłoszone 2026-06-03, Oskar buduje równolegle]: zamysł, by kolejne etapy **bramkować danymi pozyskiwanymi od NPC/sceny**. Dotąd dwa warianty zbudowane w apce: **(a) liczba-kod** — `30` od Albrechta (TR, `type:'code'`, #77); **(b) obserwacja** — kolor szaty + atrybut Jordana (KZ, `type:'observe'`, #77). Do ustalenia jako **system**: gdzie dane się zbierają, czy się składają (np. w jeden kod finałowy), jak weryfikuje MG, jak to się ma do istniejących kluczy finału (Z7 nagłówki / Z11 Tabela 1). Agent nie projektuje treści — czeka na ramę Oskara, potem spina dokumenty.
