...
77. **Tryb hybrydowy fizyczne/online** — ROZSTRZYGNIĘTE [2026-06-03, Oskar]: briefy → **apka online za kodami dostępu** (gracz wpisuje kod → uruchamia aktualny krok fabularny na telefonie); **fizyczne tylko rekwizyty**: deszyfrowniki, mapy, pergamin (lista TR), przepis+karta przypraw, tabele 1/2, polichromie, list zapieczętowany, **kalimba**. **Melodia-wzorzec Z4/Z8 odtwarzana z telefonu graczy** — kalimba zostaje fizycznym instrumentem (gracz gra i czyta cyfry). **Kody:** MG (start + handoff opcjonalnej), aktor (Z2→Z3/Z3Z, Z3→Z3b), reszta = **rozwiązanie poprzedniej zagadki** (apka auto-waliduje). Każda podstrona = **ramka-przypominajka rekwizytu** (fail-safe, gdyby prowadzący zapomniał wręczyć). Dystrybucja druków = **pakiet startowy + wręczenia w scenach**. Zakres rozstrzygnięcia = dokument/spec; **budowa aplikacji = osobna faza** (luki: stos, offline/PWA, format kodów). Szczegóły: `mechanics/hybryda-online.md`.
