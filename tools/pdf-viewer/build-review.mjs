...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .group-picker-label {
    font-size: 12px; color: var(--muted); text-transform: uppercase; letter-spacing: .5px; margin-bottom: 8px;
  }
  .group-picker {
  ...
  .group-chip {
    ...
    cursor: pointer; font-size: 13px; font-weight: 700; padding: 8px 12px;
    border-radius: 999px; line-height: 1; transition: border-color .12s, background .12s;
    position: relative;
  }
  .group-chip:hover { border-color: var(--gcolor, var(--accent)); }
  .group-chip.active {
    border-color: var(--gcolor, var(--accent));
    background: color-mix(in srgb, var(--gcolor, var(--accent)) 18%, var(--surface));
  }
  .group-chip .dot {
    display: inline-block; width: 10px; height: 10px; border-radius: 50%;
    background: var(--gcolor, #888); margin-right: 5px; vertical-align: middle;
...
<div class="topbar">
  <div class="topbar-inner">
    ...
    <div>
      ...
      <div class="sub">Przygoda dla Piotra — wybierz grupę</div>
    </div>
  </div>
</div>
...
<div class="group-picker-wrap">
  <div class="group-picker-label">Wybierz grupę:</div>
...
// --- Group picker ---
let activeGroup = localStorage.getItem('review-group') || 'G1';
const picker = document.getElementById('group-picker');
for (const [gid, g] of Object.entries(DATA.groups)) {
  const btn = document.createElement('button');
  btn.className = 'group-chip' + (gid === activeGroup ? ' active' : '');
  btn.dataset.group = gid;
  btn.style.setProperty('--gcolor', g.hex);
  btn.innerHTML = '<span class="dot"></span>' + gid + ' ' + g.kolor;
  btn.addEventListener('click', () => {
    activeGroup = gid;
    localStorage.setItem('review-group', gid);
    document.querySelectorAll('.group-chip').forEach(b => b.classList.toggle('active', b.dataset.group === gid));
    renderEtapy();
  });
...
function renderEtapy() {
  ...
  for (const etap of DATA.etapy) {
    ...
    // Pliki
    const files = etap.filesPerGroup[activeGroup] || [];
    if (etap.typ === 'fork-frakcja') {
      // Pokaż obie frakcje obok siebie
      ...
      // TR files = pliki dla jednej z grup TR (np. G1)
      const trFiles = etap.filesPerGroup['G1'] || [];
      trDiv.appendChild(makePlikGrid(trFiles));
      ...
      const kzFiles = etap.filesPerGroup['G6'] || [];
      kzDiv.appendChild(makePlikGrid(kzFiles));

      wrap.appendChild(trDiv); wrap.appendChild(kzDiv);
      section.appendChild(wrap);
    } else if (etap.typ === 'fork-frakcja-grupa') {
      // Pokaż tylko pliki wybranej grupy
      const label = document.createElement('div');
      label.style.cssText = 'font-size:12px;color:var(--muted);margin-bottom:8px;';
      const zagLabel = { Z4:'Z4 kalimba', Z5:'Z5 piernik sensoryczny', Z6:'Z6 herby hanzeatyckie', Z8:'Z8 melodia zakonu', Z9:'Z9 piernik sensoryczny', Z10:'Z10 polichromie Biedronki' };
      label.textContent = activeGroup + ': ' + (zagLabel[g.opcjonalna] || g.opcjonalna);
      section.appendChild(label);
      section.appendChild(makePlikGrid(files));
    } else {
      // wspolny lub wspolny-warianty
      if (etap.typ === 'wspolny-warianty') {
        const label = document.createElement('div');
        label.style.cssText = 'font-size:12px;color:var(--muted);margin-bottom:8px;';
        label.textContent = 'Pokazuję: ' + activeGroup + ' (' + g.kolor + ')';
        section.appendChild(label);
      }
      section.appendChild(makePlikGrid(files));
    }
