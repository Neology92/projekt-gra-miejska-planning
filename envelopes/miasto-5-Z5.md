# Koperta `miasto-5-Z5` — The Baker's Jars (draft)

Frakcja: **miasto** (Tajna Rada) · Pozycja na ścieżce (nr): **5** · Zadanie: **Z5** (sensoryczna z piernikiem).
Źródło treści: `puzzles/zagadka-sensoryczna.md`. Status: **draft [2026-06-02, v2 — EN + głos sceptyka + spec 6-słojowa + konwencja głosu]** — awaiting Oskar's review.

> **Language:** English (player-facing, #70). Planning notes in Polish below.
> **Voice:** scene narration **from inside the couriers' heads**, 2nd person „you" (#67); the baker's spoken plea = roman (words of the world); MG instruction = separate boxed frame. *(Game-wide envelope convention — `mechanics/koperty-mg.md`; wzorzec = `miasto-5-Z4`.)*
> **Typography:** narration = *italic* (inside the head); baker's words / the recipe = roman.

## Meta — montaż

- **Kolor w nazwie?** Nie — koperta **identyczna dla wszystkich grup mieszczan z zestawem B** (Z3+Z5; 2 grupy — `concept/03-faza-2b-asynchroniczne.md`). Gdyby treść różniła się per kolor → `miasto-<kolor>-5-Z5.md`.
- **Oś opcjonalna:** to jeden z trzech wariantów pozycji 5 miasta (Z4/Z5/Z6) — każdy daje **wymienialny** klucz do finału. Pozostałe: `miasto-5-Z4` (melodia), `miasto-5-Z6` (herby).
- **Kiedy wręczyć:** po rozwiązaniu **Z3** (infiltracja zamku), gdy grupa zestawu B wraca do MG. To koperta otwierająca część opcjonalną.
...
1. **Brief narracyjny** (gotowy tekst niżej) — scena + głos piekarza-sceptyka.
2. **Cel + wskazówka** wplecione w narrację (zasady badania słojów + rola przepisu).
3. **Ramka MG** — oddanie wypełnionej karty.

> ⚠ Klucz do finału (co konkretnie w kopercie nagrodowej) — wciąż otwarty (#9 w `zagadka-sensoryczna.md`); tu tylko generyczny hak („zapłata + list, co czynić dalej"), bez spoilera finału.

## TEXT — Z5 (player reads this)
...
> Friends — forgive the scrawl, I've no hour to spare. A fool of an apprentice wiped my labels and set the lids back wrong, and I'll not heat the oven on a guess: one wrong root sours the whole batch. You've keen senses and you're from here, so you'll know them — cinnamon, cloves, ginger and cardamom, by their scent alone. Take their measure however you can, only do not open them, for a spice once aired is spoiled. And hold to my rule: it names every root and meal a piernik takes — but not all of them stand here on the bench tonight, so you'll have more names than jars. Set each jar to its name, and let the rest be.
>
...
*Seven jars, and no two alike — if your senses are keen enough to part them. Trust what they tell you, and set the right name to each letter on his card. His recipe names every ingredient a piernik takes; only some of them stand here in the jars before you, so a few names will have no jar at all — the recipe is your guide to what to expect.*
...
- **Głos piekarza-sceptyka [brief Oskara 2026-06-02, MVP — `zagadka-sensoryczna.md` linie 134–149]:** filar tonalny „TR/kupcy = źli" (`lore/fakty-vs-fabula.md`, `concept/00-overview.md §Wymowa moralna`). Wymowa: **„rewolucja wysokich sfer"** — bunt otwiera wielkim kupcom rynki, drobnemu rzemieślnikowi grozi katastrofą. *Rich stay rich, poor stay poor.*
...
- **Brief NAZYWA 4 przyprawy słoikowe wprost** („cinnamon, cloves, ginger and cardamom, by their scent alone") — by konkretne przedmioty padły w zajawce; mąki/miód NIE są nazwane (gracz dochodzi sam). **Model przepis=pełna lista [Oskar 2026-06-03]:** karta przepisu wylicza WSZYSTKIE składniki piernika (pula nazw); słoiki to podzbiór, brief wprost mówi „more names than jars / let the rest be". Dystraktory (nutmeg, pepper, candied orange peel) odrzuca **brak słoja (zmysły)**, NIE „brak w przepisie" (są w nim). Poniższa notka „celowo OGÓLNA" dotyczy tylko KANAŁÓW sensorycznych i pary mąk (nadal nieujawniane).
- **Mechanika w narracji — kanały/mąki celowo OGÓLNE [Oskar 2026-06-02]:** brief NIE wylicza kanałów ani nie rozwiązuje pary mąk — gracz ma sam dojść. **Usunięte podpowiadacze:** „weigh in the hand / shake for rattle-pour-slosh / scentless dust / heavier in the jar". Zostaje ogólne „take their measure however you can" + „trust what they tell you" + „do not open" (zasada) + „recipe is your guide" (na dystraktory). Para mąk: proporcja rye:wheat 3:1 żyje **wyłącznie na karcie przepisu** (player-prop) — gracz sam kojarzy cięższy→żyto, koperta tego nie zdradza. Spec/kanały → `zagadka-sensoryczna.md`.
...
- **Lustro Z9 (krzyżacy):** `krzyzacy-4-Z9` — INNY framing (brat-kucharz, duma niemiecko-hanzeatycka, NIE sceptyk buntu). Wymaga osobnego przejścia EN+głos + spec 6-słojowa. NIE kopiować 1:1.
